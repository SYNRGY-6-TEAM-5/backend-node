...
class AirlineApi {
  ...
  routes() {
     ...
     *         description: Product not found
     */
    this.router.get('/:car_id', AirlineController.show); // /api/books/1 -> /api/books/:id READ
    this.router.put(
      '/:car_id',
      [AuthMiddleware.authorizeAdmin, Media.upload.single('image')],
      AirlineController.update
    ); // /api/books/1 -> /api/books/:car_id UPDATE
    this.router.delete('/:car_id', AuthMiddleware.authorizeAdmin, AirlineController.delete); // /api/books/1 -> /api/books/:id DELETE
